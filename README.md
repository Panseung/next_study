...
html을 유저 브라우저에서 직접 실시간으로 생성하는 것
...
변수의 값이 변경되면 실행할 코드가 실행됨
(빈 배열을 넣으면 초기 한 번만 실행)



실행 순서는 html 을 먼저 렌더링 한 후에
useEffect가 실행됨 -> 시간차가 있음

##### Q: 그럼 ajax로 데이터를 가져 온 후에 html을 보여줘야 하는 것이 순서가 아닌가?

##### A: 데이터를 받아 오기 전 임시 html을 (로딩 화면) 보여준 후 데이터를 받아오면 다시 html에 꽂아넣기





### 31. loading.js / error.js / not-found.js

Next는 기본적으로 SSR이다보니 페이지를 렌더링하는데
CSR보다 비교적 늦을 수 있는데 UX개선을 위해
렌더링해야 하는 페이지에 필요한 데이터가 모두 로드 되기전에
로딩UI를 보여주는 것이 좋다.

react에서는 Suspense태그를 사용
```react
<Suspense fallback={ loading.js내용 }>
	(page.js내용)
</Suspense>
```
...
Next에서는 별도의 코드 작성 없이
보여주고자 하는 page.js와 같은 폴더에
loading.js 혹은 error.js 파일을 작성하면
렌더링이 모두 되기 전, 혹은 에러가 생기면
해당 파일을 대신하여 보여준다.

!!!!!페이지 전체를 error.js로 대체하는 것이 아니라
보여주고자 했던 page.js가 차지하는 영역 부분만
error.js로 대체하는 방식! -> 다른 부분들은 정상적으로 동작

!!!!! 같은 폴더 내에 error.js가 없으면 상위 폴더들을 뒤져가며 가까운
error.js를 찾아서 대체함
따라서 개별적으로 에러 처리를 해야하는 경우가 아니면 
하나의 error.js를 최상위 폴더에 작성하면 별 다른 error처리를 할 필요가 없음

최상위에 있는 layout.js의 에러는 체크를 못하는데
이때는 global-error.js를 작성



404에러(없는 URL로 접속한 경우)가 뜨는 경우 next에서 기본적으로 제공하는
404에러페이지를 보여주게 되는데 URL에서 id나 쿼리쪽 부분을 잘못 입력한 경우는
404에러 페이지를 보여주는 것이 아니라 에러가 발생하게 된다. 

이때는 not-found.js를 작성하고 에러가 발생할 수 있는 코드에 다음과 같이 작성한다
```react
let result = ~~~~
(result가 없는 경우 에러)
if (result === null) {
